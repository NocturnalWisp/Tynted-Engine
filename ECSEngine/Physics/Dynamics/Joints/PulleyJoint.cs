	...
	public class PulleyJoint : Joint
	{
		...
		internal override void SolveVelocityConstraints(TimeStep step)
		{
			...
			if (_state == LimitState.AtUpperLimit)
			{
				...
				_impulse = Box2DNet.Common.Math.Max(0.0f, _impulse + impulse);
			...
			if (_limitState1 == LimitState.AtUpperLimit)
			{
				...
				_limitImpulse1 = Box2DNet.Common.Math.Max(0.0f, _limitImpulse1 + impulse);
			...
			if (_limitState2 == LimitState.AtUpperLimit)
			{
				...
				_limitImpulse2 = Box2DNet.Common.Math.Max(0.0f, _limitImpulse2 + impulse);
